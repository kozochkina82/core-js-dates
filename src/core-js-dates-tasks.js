...
function getDayName(date) {
  ...
  const myDay = weekday[myDate.getDay()];
  return myDay;
}
...
function getNextFriday(date) {
  const myDateNumber = date.getDay();
  const daysToAdd = 5 - myDateNumber + 7 * (myDateNumber >= 5 ? 1 : 0);
...
function formatDate(date) {
  ...
  const minute = myDate.getUTCMinutes();
  ...
  return `${month}/${day}/${year}, ${formattedHour}:${formattedMinute}:${formattedSecond} ${period}`;
}
...
function getWeekNumberByDate(date) {
  ...
  myDate.setDate(myDate.getDate() + 4 - ((myDate.getDay() + 6) % 7));
  const yearStart = new Date(myDate.getFullYear(), 0, 4);
  yearStart.setHours(0, 0, 0, 0);
  const weekNumber =
    1 +
    Math.round(
      ((myDate - yearStart) / 86400000 - 3 + ((yearStart.getDay() + 6) % 7)) / 7
    );
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i < 120; i += 1) {
    const thirteens = new Date(year, month, 13);
    if (thirteens > date && thirteens.getDay() === 5) {
      return thirteens;
    }
    month += 1;
    if (month > 11) {
  ...
  throw new Error('Следующая пятница 13-го не найдена в пределах 10 лет');
}
...
function isLeapYear(date) {
  const year = date.getFullYear();
  const isTrue = (year % 4 === 0 && year % 100 !== 0) || year % 400 === 0;
  return isTrue;
}
